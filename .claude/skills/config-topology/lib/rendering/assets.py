...
.badge.ospf-b { color: var(--ink-faint); border-color: var(--panel-edge); font-size: 9px; }
...
/* presentAreas(data): data.links (非 admin_down かつ area あり) と data.segments (area あり) から
   ...
   凡例用は描画される links/segments の area から集計。 */
...
function render() {
  ...
  if (showBgp) {
    ...
    for (const e of DATA.extPeers) {
      ...
      // 外部ピアは hn（neighbor IP）の 1 行のみ。AS は vbar 色・枠ラベル・ホバー title で識別する。
      // 機器ノード（hn + sub 2 行）と意図的に非対称。副ラベル行（class=sub）は存在しない。
      parts.push(`<g class="${ecls.join(" ")}" data-elem="ext" data-id="${e.id}">
        ...
        <text class="hn" x="${p.x-NODE_W/2+16}" y="${p.y-3}">${esc(truncateLabel(e.sub, extMaxc))}</text>
      </g>`);
    }
  }
  ...
  for (const [id,d] of Object.entries(DATA.devices))
    for (const i of d.ifs)
      allRows.push({ dev:d.hostname, devId:id, ifn:i.n, ip:i.ip, ip6:i.ip6, desc:i.d, st:i.st,
                     ...
                     addrs: i.addrs||[], v6list: ifV6List(i), ospf: i.ospf||null });
