...
- **表ビュー**（上部タブ）: `ADDRESSES`（インターフェース集約・IP 一覧）・`INTERFACES`（インターフェース詳細・状態・速度・description）・`STATS`（構成統計ダッシュボード：機器/IF/リンク/セグメント数・ベンダー別・AS 別・OSPF area 別・dual-stack 率・BGP/OSPF/static 件数の集計）・`CHECKS`（設計検証パネル：重複 IP・OSPF/BGP router-id 重複・MTU 不一致・BGP local_ip 未解決・到達不可 next_hop 等の設計上の注意点を severity / kind / message / refs でリスト表示。0 件なら「問題は検出されませんでした」）。`DIFF`（条件付き：`--diff-against` 指定時のみ表示 — 前回との差分を devices/interfaces/links/segments/routing_bgp/routing_ospf/routing_static の固定順で added(+)/removed(-)/changed(~) のセクションと件数サマリで表示。差分ゼロなら「差分なし」を明示）。
