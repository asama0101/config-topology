...
```
- **図ビュー**（上部タブ）: `PHYSICAL`（L1物理＝機器+リンク+セグメント）＋プロトコル別（`BGP`・`OSPF` … `routing` キーから動的生成。例: BGP ネイバー隣接・OSPF エリア別）。OSPF ビューでは OSPF 参加 loopback を機器ノード脇の扇状スタブ円（area 色）として表示する（凡例 dim 連動は対象外）。
