...
- **表ビュー**（上部タブ）: `ADDRESSES`（インターフェース集約・IP 一覧）・`INTERFACES`（インターフェース詳細・状態・速度・description）・`STATS`（構成統計ダッシュボード：機器/IF/リンク/セグメント数・ベンダー別・AS 別・OSPF area 別・dual-stack 率・BGP/OSPF/static 件数の集計）。
