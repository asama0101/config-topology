...
```
- **図ビュー**（上部タブ）: `PHYSICAL`（L1物理＝機器+リンク+セグメント）＋プロトコル別（`BGP`・`OSPF` … `routing` キーから動的生成。例: BGP ネイバー隣接・OSPF エリア別）。OSPF ビューでは OSPF 参加 loopback を機器ノード脇に **segment と同一表記のノード**（点線楕円＋loopback サブネットのみ＋area バッジ）として表示する（**非選択・IF 名非表示**＝segment と同じ・凡例 dim 連動は対象外）。
