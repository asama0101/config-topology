...
```
- **図ビュー**（上部タブ）: `PHYSICAL`（L1物理＝機器+リンク+セグメント）＋プロトコル別（`BGP`・`OSPF` … `routing` キーから動的生成。例: BGP ネイバー隣接・OSPF エリア別）。OSPF ビューでは OSPF 参加 loopback を機器ノード脇に **segment 様式のノード**（点線楕円＋loopback サブネット＋area バッジ）として表示する。**ホバーで IF 名 + IP を表示**し、**クリックで親デバイスを選択**できる（親デバイスの詳細パネルを表示・凡例 dim 連動は対象外）。
