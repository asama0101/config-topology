...
把握に複数ファイルを要する勘所:
- **正規化モデル `lib/parsers/base.py` の `Device`** がパイプライン全体の中心。パーサはここに正規化し、
  build / render はこのモデル（→ topology dict）しか見ない。新ベンダーはこのモデルに合わせるだけ。
- **パーサ registry `lib/parsers/__init__.py`**: `detect` の特異度が高い順に試行（JunOS→IOS）。
...
python3 "$SKILL/scripts/parse_configs.py" <paths...>          # 正規化 Device を JSON 確認（デバッグ）
python3 "$SKILL/scripts/build_topology.py" [paths...] -o ./topology   # paths 省略時は workspace/ を走査
python3 "$SKILL/scripts/render_topology.py" ./topology -o ./topology.html
```
