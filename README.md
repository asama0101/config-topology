...
- **結線の自動推論** — 各インターフェースの IP / サブネット一致でリンクを推論（2 機器=リンク、3 機器以上=共有セグメント、単独=スタブ）。BGP は対向を解決して ebgp/ibgp を判定。
- **編集可能なレイヤー別 YAML 正本** — 中間表現は人手で補正・注記できる層別 YAML。読込時に ID 参照整合を検証し、再描画する round-trip が可能。
- **自己完結・決定的な HTML 出力** — 外部依存ゼロ・`file://` で開ける単一 HTML。レイヤー別ビュー切替・検索・ズーム/パンに対応し、同一入力 → 同一出力。
